...
.menu-cleaner-reorder-column-section.drag-over-section {
  ...
  function renderHideView() {
    ...
    doc.querySelectorAll('.menu-cleaner-category-body').forEach(function (eb) {
      if (!eb.classList.contains('collapsed')) expandedGroups[eb.dataset.group] = true;
    });
    ...
    for (const eg in expandedGroups) {
      const catBody = doc.querySelector('.menu-cleaner-category-body[data-group="' + eg + '"]');
      if (catBody) {
